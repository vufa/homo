...
import (
	...
	bb "github.com/countstarlight/homo/homo-hub/broker"
	"github.com/countstarlight/homo/homo-hub/config"
	"github.com/countstarlight/homo/homo-hub/persist"
	"github.com/countstarlight/homo/homo-hub/rule"
...
func prepare() (res *resources, err error) {
	...
	res.broker, err = bb.NewBroker(c, res.factory, nil, logger.S)
	if err != nil {
		return
	}
	res.rules, err = rule.NewManager(make([]config.Subscription, 0), res.broker, nil, logger.S)
	if err != nil {
		return
	}
	res.sessions, err = NewManager(c, res.broker.Flow, res.rules, res.factory, logger.S)
	if err != nil {
		return
	}
	res.rules.Start()
