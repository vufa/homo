	...
	cfg := ComposeAppConfig{
		...
		Services: map[string]ComposeService{
			"test-hub": ComposeService{
				Image: "hub.test.com/homo/homo-hub",
				...
				Volumes: []ServiceVolume{
					ServiceVolume{
						Source:   "var/db/homo/test-hub-conf",
						Target:   "/etc/homo",
						ReadOnly: false,
					},
				},
			...
			"test-timer": ComposeService{
				Image:     "hub.test.com/homo/homo-timer",
				...
				Volumes: []ServiceVolume{
					ServiceVolume{
						Source:   "var/db/homo/test-timer-conf",
						Target:   "/etc/homo",
						ReadOnly: true,
					},
				},
...
services:
  test-hub:
    ...
    ports:
      - 1883:1883
    volumes:
      - var/db/homo/test-hub-conf:/etc/homo
    command:
      - '-c'
      - conf/conf.yml
    environment:
...
volumes:
  - name: test-hub-conf
    path: var/db/homo/test-hub-conf
  - name: test-timer-conf
    path: var/db/homo/test-timer-conf`
