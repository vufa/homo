...
func (s *Server) Start() error {
	...
	if uri.Scheme == "unix" {
		...
		if err != nil {
			logger.S.Errorf("failed to make directory %s : %s", dir, err.Error())
		}
	}
	...
	logger.S.Infof("api server is listening at: %s", s.conf.Address)
	go func() {
		if err := s.svr.Serve(listener); err != nil {
			logger.S.Infof("api server shutdown: %v", err)
		}
	}()
	return nil
}
