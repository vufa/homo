...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='convert raw data to rasa nlu format json file')
    parser.add_argument('-i',
                        '--input',
                        type=str,
                        default=os.path.join("data", "zh", "raw", "raw_data.txt"),
                        help='raw data file path')
    parser.add_argument('-o',
                        '--output',
                        type=str,
                        default=os.path.join("data", "zh", "train_data.json"),
                        help='output file path')
    args = parser.parse_args()
    with open(args.input) as f, open(args.output, "w") as f2:
        train_set = {}
