...
func NewFunction(cfg FunctionInfo, p Producer, log *zap.SugaredLogger) *Function {
	f := &Function{
		...
		log: log.With("function", cfg.Name),
	}
...
func (f *Function) call(i Instance, in *homo.FunctionMessage, c func(in, out *homo.FunctionMessage, err error)) (*homo.FunctionMessage, error) {
	out, err := i.Call(in)
	if err != nil {
